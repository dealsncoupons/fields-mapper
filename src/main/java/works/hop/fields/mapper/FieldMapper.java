...
public class FieldMapper {
    ...
    public <T> T map(Object source, Class<T> type) {
        ...
        mapping.forEach((key, value) -> {
            ...
            value.mapSupplier();
            value.mapConsumer();
            value.mapAtoB.consumer.accept(value.mapAtoB.supplier.get());
        });
        return target;
    }
    ...
    private static class MappingInfo {
        ...
        Class<?> paramType;
        Object targetObj;
        MapAtoB mapAtoB = new MapAtoB();
        char[] delimiters = new char[]{'[', '.'};
        Pattern mapAccessorPattern = Pattern.compile("([\\w_$]+)\\[([\\w_$]+?)\\]");
